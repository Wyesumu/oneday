...
def index():
	...
	if 'username' in flask.session:
		...
		username = None
	print(username)
	return flask.render_template("index.html", images=images, username=username, time_left = time_left())
...
#
	if 'logged' in flask.session: # allow access only for logged users
		conn=connection()
		with conn:
			...
			if flask.request.method == 'POST':
				...
				if allowed_file(files): # if file extension is allowed
					...
					for file in files: #save every file in Image folder (which contains original of uploaded images)
						...
						file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename[-1])) #save this file

						#load saved file and resize it to 700x500 (it will be used inside post)
						with Image.open(os.path.join(app.config['UPLOAD_FOLDER'], filename[-1])) as image:
					...
					image = resize_and_crop(os.path.join(app.config['UPLOAD_FOLDER'], filename[0]), (256,256), "middle")
		...
		flask.flash("Необходимо войти или зарегистрироваться, чтобы иметь возможность загружать работы")
		return flask.redirect(flask.url_for("index"))
...
def stats():
	if 'logged' in flask.session: #only logged users allow
		if flask.request.method == "POST":
		...
		flask.flash("Необходимо войти или зарегистрироваться, чтобы просматривать статистику пользователей")
		return flask.redirect(flask.url_for("index"))
...
def user_gallery(username):
	...
	return flask.render_template("index.html", images=images, username=session_username)
...
def calendar():
	if 'username' in flask.session:
		...
		username = None
	return flask.render_template("cal.html", username = username)
